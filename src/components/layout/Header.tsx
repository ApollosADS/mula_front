        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
          <div className="flex justify-between items-center h-16 sm:h-20 md:h-24">
            ...
            <nav className="hidden md:flex space-x-6 lg:space-x-8 xl:space-x-10">
              {navLinks.map((link) => (
                <Link key={link.name} href={link.path} className={`text-sm md:text-base uppercase tracking-wide transition-colors duration-200 ${isActive(link.path)}`}>
                  {link.name}
                </Link>
              ))}
            </nav>
            ...
            <div className="hidden md:flex items-center">
              <Link href="/panier" className="flex items-center justify-center bg-mula-black hover:bg-mula-red text-white p-3 rounded-full transition-all duration-300 relative">
